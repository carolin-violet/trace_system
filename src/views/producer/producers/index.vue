    ...
    >
      ...
      <el-table-column label="操作" width="600" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="mini " type="primary" @click="getSummary(scope.$index, scope.row)">查看生产信息汇总</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getUsers('producer').then(response => {
        this.list = response
    ...
    getSummary(index, row) {
      this.$router.push({
        path: '/producer/produceSummary',
        query: {
          user_id: row.user_id
        }
      })
